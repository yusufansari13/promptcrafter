...
- Do not include any markdown formatting blocks like \`\`\` or introductory text. Just return the sanitized prompt.`;
  ...
  try {
    ...
    if (error?.status === 429 || error?.message?.includes('429') || error?.message?.includes('RESOURCE_EXHAUSTED') || error?.message?.includes('quota')) {
      throw new Error('You have exceeded your Gemini API quota. Please check your plan and billing details, or try again later.');
    }
    throw new Error('Failed to sanitize prompt. Please try again.');
  }
}
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: {
        parts: [
          ...imageParts,
          { text: 'Analyze this image and generate the maximum fidelity restoration prompt according to the system instructions.' }
        ]
      },
      config: {
        systemInstruction,
    ...
    return parsePromptResponse(response.text || '');
  } catch (error: any) {
...
Make sure the look and feel of the output image matches the requested style adherence. Do not include markdown formatting blocks like \`\`\`.`;
  ...
  try {
    ...
    if (synthesisMode) {
      parts.push({ text: "Image 1 (SUBJECT REFERENCE):" });
      parts.push({ inlineData: { data: synthesisMode.subjectImage.base64, mimeType: synthesisMode.subjectImage.mimeType } });
      parts.push({ text: "Image 2 (STYLE REFERENCE):" });
      parts.push({ inlineData: { data: synthesisMode.styleImage.base64, mimeType: synthesisMode.styleImage.mimeType } });
      parts.push({ text: "Analyze Image 1 for the SUBJECT and COMPOSITION. Analyze Image 2 for the ARTISTIC STYLE, MEDIUM, and VIBE. Merge them together according to the system instructions." });
    } else {
      const imageParts = images.map(img => ({
        inlineData: {
          data: img.base64,
      ...
      parts = [
        ...imageParts,
        { text: 'Analyze this image and generate the prompt according to the system instructions.' }
      ];
    }

    const response = await ai.models.generateContent({
      model: 'gemini-3.1-pro-preview', // Using the best model for complex reasoning and vision
      contents: { parts },
      config: {
        systemInstruction,
        temperature: temperature, // Dynamically set based on style adherence
      },
    });
