...
- Do not include any markdown formatting blocks like \`\`\` or introductory text. Just return the sanitized prompt.`;
  ...
  try {
    const response = await ai.chat.completions.create({ model: 'gpt-5.4', messages: [{ role: 'system', content: systemInstruction }, { role: 'user', content: originalPrompt }], temperature: 0.4 });
...
Do not include markdown formatting blocks like \`\`\`.`;
  ...
  try {
    ...
    const response = await ai.chat.completions.create({ model: 'gpt-5.4', messages: [{ role: 'system', content: systemInstruction }, { role: 'user', content: [ ...openaiParts, { type: 'text' as const, text: 'Analyze this image and generate the maximum fidelity restoration prompt according to the system instructions.' } ] }], temperature: 0.1 });
